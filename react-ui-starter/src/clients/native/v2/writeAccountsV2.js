...
const createJokeInstruction_PDA_version = async ({ connection, sendTransaction, authorPubkey, joke }) => {
  ...
  // 2. Crafting Keys (see ../target/idl/joketoearn.json)
...
const upvoteJokeV2 = async ({ connection, sendTransaction, voterPubkey, jokeAddress }) => {
  // 1. Crafting Keys (see ../target/idl/joketoearn.json)
  const jokeAccKey = { pubkey: jokeAddress, isSigner: false, isWritable: true };
  const voterKey = { pubkey: voterPubkey, isSigner: true, isWritable: true };
  const sysProgramKey =  { pubkey: SystemProgram.programId, isSigner: false, isWritable: false };
  const keys = [jokeAccKey, voterKey, sysProgramKey];

  // 2. Crafting Instruction
  const instruction = new TransactionInstruction({
    programId: programAddress,
    data: upvoteJokeInstruxBuffer("v2"),
    keys: keys
  });
